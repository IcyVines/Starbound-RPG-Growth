...
function init()
  ...
  self.projectileList = {"ivrpg_elementressfire", "ivrpg_elementressice", "ivrpg_elementresselectric"}
  self.projectileList2 = {"molotovflame", "icetrail", "electrictrail"}
  self.levelList = {1, 1, 1}
...
function action1(skipCheck)
  ...
  elseif self.element == "fire" then
    world.spawnProjectile(self.projectileList[self.elementMod], {mcontroller.xPosition(), mcontroller.yPosition() - 0.5}, self.id, world.distance(tech.aimPosition(), mcontroller.position()), false, {power = 4 * bonusDamage, powerMultiplier = status.stat("powerMultiplier"), speed = 20, timeToLive = 0.5 + bonusTime})
