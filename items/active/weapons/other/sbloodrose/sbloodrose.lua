...
function init()
  ...
  self.weapon:addTransformationGroup("weapon", {0,0}, util.toRadians(config.getParameter("baseWeaponRotation", 0)), {0,2})
...
function Weapon:updateAim()
  for _,group in pairs(self.transformationGroups) do
    ...
    if (group.name == "weapon") then
      animator.scaleTransformationGroup(group.name, {0.25, 0.25})
      animator.translateTransformationGroup(group.name, {0, 1.5})
    end
