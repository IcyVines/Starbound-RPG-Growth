...
function MeleeCombo:init()
  ...
  self.previousElementType = self.weapon.elementalType
...
function MeleeCombo:update(dt, fireMode, shiftHeld)
  ...
  if self.elementalType ~= self.previousElementType then
    self.previousElementType = self.weapon.elementalType
    self.damageConfig.damageSourceKind = self.weapon.elementalType == "physical" and "broadsword" or (self.weapon.elementalType .. "broadsword")
    self:computeDamageAndCooldowns()
  end
