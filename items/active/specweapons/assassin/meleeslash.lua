...
function MeleeSlash:update(dt, fireMode, shiftHeld)
  ...
  self.weaponOne = world.entityHandItem(activeItem.ownerEntityId(), "primary")
  self.weaponTwo = world.entityHandItem(activeItem.ownerEntityId(), "alt")
end
...
function MeleeSlash:fire()
  ...
  if self.weaponOne and self.weaponTwo and root.itemHasTag(self.weaponOne, "weapon") and root.itemHasTag(self.weaponTwo, "weapon") then
    ...
    self.damageConfig.baseDamage = (self.baseDps + 3) * self.fireTime
  end
