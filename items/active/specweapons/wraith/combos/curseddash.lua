...
function DashSlash:dash()
  ...
  util.wait(self.stances.dash.duration, function()
    if status.overConsumeResource("energy", self.energyCost) then
      ...
      mcontroller.controlApproachVelocity(self:faceVector(self.stances.dash.velocity), 2000)
    end
...
function DashSlash:faceVector(vector)
  return {vector[1] * self.weapon.aimDirection, vector[2] * self.weapon.aimAngle}
end
