...
function init()
  ...
  self.initComboSteps = self.comboSteps
...
function update(dt, fireMode, shiftHeld)
  ...
  self.comboBreaker = status.statPositive("ivrpguccombobreaker") and self.name == "roguesiphonclaw3s"
  ...
  if self.comboBreaker then
    self.comboSteps = self.initComboSteps - 1
    if self.comboStep > self.comboSteps then resetFistCombo() end
  elseif self.name == "roguesiphonclaw3s" then
    self.comboSteps = self.initComboSteps
  end
...
function triggerComboAttack(comboStep)
  if self.primaryAbility:canStartAttack() then
    ...
    if comboStep == self.comboSteps then
      self.comboFinisher:startAttack()
    elseif comboStep >= self.comboSteps and self.comboBreaker then
      self.comboFinisher:startAttack()
    else
      self.primaryAbility:startAttack()
    end
